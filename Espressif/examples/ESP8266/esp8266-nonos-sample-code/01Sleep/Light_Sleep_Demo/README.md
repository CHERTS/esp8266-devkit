...
##FOR VERIFY: 
When the board sleep in light-moode,it can keep connected  to AP, send message and receive messages.The current would be about 7.3 mA when the board is sleeping and it would be about 1.2 mA when the board send and receive message.In this example, we create a tcp server after the board wakeup and you can check the received infomation by serial.
