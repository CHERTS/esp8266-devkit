...
@set comport=COM2
@set curdir=%cd%
@set eagle_app_v6_out="app.out"
@set eagle_app_v6_out_dir="build"
@set irom0text_addr="0x40000"
@set xtensa_tool_root="C:\Espressif\xtensa-lx106-elf\bin"
@set sdk_base="C:\Espressif\ESP8266_SDK"
  ...
  if exist %eagle_app_v6_out% (
    ...
    if exist eagle.app.flash.bin (
      ...
      if exist eagle.app.flash.bin (
        ...
        %devkit_utils%\esptool-ck.exe -cp %comport% -cd ck -cb 256000 -ca 0x7e000 -cf %sdk_base%\bin\blank.bin -v
      )
    ) else (
