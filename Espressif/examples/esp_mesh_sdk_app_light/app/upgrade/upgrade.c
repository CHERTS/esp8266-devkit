...
void ICACHE_FLASH_ATTR
system_upgrade_erase_flash(uint32 sumlength)
{
	uint8 i = 0;
    uint16 sec_block = ((sumlength/SPI_FLASH_SEC_SIZE) + 1);
	os_printf("SUM LEN : %d \r\n",sumlength);
	os_printf("SEC BLOCE NUM: %d \r\n",sec_block);
	os_printf("SEC BEGIN: %d \r\n",upgrade->fw_bin_sec);
	os_printf("=========================\r\n");
    pp_soft_wdt_stop();
	for (i =0; i < sec_block; i++) {
		spi_flash_erase_sector(upgrade->fw_bin_sec + i);
	}
	pp_soft_wdt_restart();
}
...
upgrade_download(void *arg, char *pusrdata, unsigned short length)
{
    ...
    if (totallength == 0 && (ptr = (char *)os_strstr(pusrdata, "\r\n\r\n")) != NULL &&
		...
		if (sumlength != 0) {
			sec_block = sumlength / SPI_FLASH_SEC_SIZE + 1;
			pp_soft_wdt_stop();
			for (i =0; i < sec_block; i++) {
				spi_flash_erase_sector(upgrade->fw_bin_sec + i);
			}
        ...
        if (length != 0) {
			if(os_memcmp(ptr + 4, A_buf, 2) != 0 && os_memcmp(ptr + 4, B_buf, 2) != 0) {
				UPGRADE_DBG("error bin file\n");
...
#endif
{
    ...
    if (upgrade_conn != NULL) {
        ...
        if (upgrade_conn->proto.tcp != NULL) {
            ...
            UPGRADE_DBG("system_upgrade_start\n");
