...
light_set_aim_r(uint32 r)
{
    ...
    //if(change_finish){
        //duty_step[LIGHT_RED] = ABS_MINUS( target_duty[LIGHT_RED] , pwm_get_duty(LIGHT_RED) )/(150/min_ms);
...
//step 1:  process the light course.
    ...
    for(i=0;i<PWM_CHANNEL;i++){	
            if( cur_ctrl_mode ==0 )
            {
                   ...
                   // if( ABS_MINUS(duty_now[i],target_duty[i])<20 )
                    //duty_now[i] = target_duty[i];
            }
            else if (cur_ctrl_mode ==1)
            {
                    ...
                    {
                            if(duty_step[i].plus_minus == 0) duty_now[i]++;  else duty_now[i]--; 
...
//step2 ctrl val according to current ctrl mode.        
        ...
        else if (cur_ctrl_mode ==1)
        {
                ...
                //os_printf("dutystep:%u,%u,%u\r\n", duty_step[0].integer,duty_step[1].integer,duty_step[2].integer);
        }
